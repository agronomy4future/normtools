...
#' New columns are added next to the existing columns and named according to the normalization type, such as `Normalized_yield_col`.
#'
#' Code source: https://github.com/agronomy4future/normtools
#'
#' @export
#'
#' @examples
#' ### Example usage with z-test normalization
#' ## to install normtool package
...
#'
  ...
  # Ensure the env_cols columns are treated as factors
  df[env_cols] = lapply(df[env_cols], as.factor)

  # Ensure yield_cols exist and are numeric
  for (yield_col in yield_cols) {
    if (!yield_col %in% names(df)) {
      stop(paste("Column", yield_col, "not found in data frame."))
    }
  ...
  # Iterate over each yield_col for normalization
  for (yield_col in yield_cols) {
    df = switch(method,
                  ...
                  df %>%
                    ...
                    mutate(
                      !!paste0("Normalized_", yield_col) :=
                        (get(yield_col) - mean(get(yield_col), na.rm = TRUE)) /
                        sd(get(yield_col), na.rm = TRUE)
                    ) %>%
                    ungroup() %>%
                    dplyr::select(all_of(env_cols), everything(),
                                  paste0("Normalized_", yield_col))
                },
                ...
                min_max_scaling = {
                  df %>%
                    ...
                    mutate(
                      !!paste0("Normalized_", yield_col) :=
                        (get(yield_col) - min(get(yield_col), na.rm = TRUE)) /
                ...
                robust_scaling = {
                  df %>%
                    ...
                    mutate(
                      !!paste0("Normalized_", yield_col) :=
                        (get(yield_col) - median(get(yield_col), na.rm = TRUE)) /
                ...
                log_transformation = {
                  df %>%
                    group_by(across(all_of(env_cols))) %>%
                    mutate(
                      !!paste0("Normalized_", yield_col) := log10(get(yield_col) + 1)  # Adding 1 to avoid log(0)
                    ) %>%
                    ungroup() %>%
                    dplyr::select(all_of(env_cols), everything(),
                                  paste0("Normalized_", yield_col))
